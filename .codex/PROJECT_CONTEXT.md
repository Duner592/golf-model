...
- `web/index.html` reads each tour's `meta.json`; when `active_events` contains more than one event, the homepage shows an event switcher and loads the selected preview from that event's `resources` paths under `web/{tour}/events/event_{event_id}/`. It must only render a full model preview when its published event name matches the current schedule entry. If an upcoming/current event is scheduled but matching model assets have not yet been published, the homepage shows a compact Upcoming card sourced directly from the schedule (event, venue, location, and start date) and withholds stale model panels/actions. Each current selected preview has a shared action bar immediately above its model/context panels: `Model Results` opens the tour table and `Event Drilldown` links to `web/event_drilldown.html` with the selected `tour`, `event_id`, and course. The homepage checks the served tour schedule and does not render these actions for tours with no current event. Off-week tour sections switch to a compact status card with a schedule link instead of rendering empty model, field, and weather panels.
- `web/recent-improvements.json` is the manually curated source for the homepage Recent Improvements strip. Keep only the latest 3-4 meaningful site/model changes, each with `date`, `title`, `description`, and optional relative `href`; do not derive this list from routine commits or generated-data refreshes.
- `web/index.html` shows Featured Current Bets only for pending current-event Outright and E/W bets, because `web/odds_value.html` evaluates only those markets. The card lists every eligible pending bet per event (player, market, price, bookmaker) with a compact EV status calculated from the matching active homepage leaderboard using the same outright/E/W assumptions as Odds & Value; it explicitly shows `EV unavailable` when a player, odds, place terms, or matching current model is unavailable. It identifies current events from the served tour schedule before deep-linking to Odds & Value with pending, tour, and tournament filters set from URL parameters. First-round-leader bets remain available through the normal pending-bets and betting-history views and must not be presented as Odds & Value bets.
...
- `web/course_details.html` is the spreadsheet-backed course lookup page. It keeps tournament and course dropdown filters so historical/non-current courses can be selected even when they are not being played this week. It renders parsed `Course Details` through `web/assets/js/course-notes.js`, which strips retired `Key Attribute` and `Insight` sections.
- `web/event_drilldown.html` is the static current-event drilldown page. It supports `tour`, `event_id`, `course`, and optional `snapshot=initial`; PGA/DP World tournament summaries and player drilldown course links point to it. The page combines served model assets (`tournament_summary.json`, course-fit weights, course-history summary, weather, leaderboard, and optional `field_teetimes.csv`) with the best matching spreadsheet course note via `web/assets/js/course-notes.js`. The Event Context yardage row uses the parsed course-note `Length` field when available, then falls back to tournament `total_yardage`. It includes directional first-round-leader analysis using model strength, course fit, R1 wave weather, tee-time data where available, and matched historical FRL bets; this is not a true FRL probability model. When matched spreadsheet rows contain usable `Pre Tournament Notes`, the page shows de-duplicated current-event-year notes inline and keeps older-year notes in a collapsed dropdown; blank, `N/A`, and `Pending` placeholders are excluded. Markdown-style checked note items such as `[x]` and `- [x]` render with the `✅` emoji.
...
- iOS-specific usability rules are isolated behind `@supports (-webkit-touch-callout: none)` plus coarse-pointer/mobile breakpoints. Do not move these rules into desktop selectors: macOS layout is intentionally unchanged. PGA and Euro leaderboards must keep their wide table inside a horizontally swipeable container instead of allowing the full page to overflow.
- `web/menu.js` injects a shared contextual breadcrumb below the fixed site header. It keeps Home, tour/event, archive, course, and betting-history routes connected without duplicating page-specific markup. When adding an event-to-course link, retain `tour`, `event_id`, and optional `snapshot` parameters so Course Details can link back to the originating Event Context.
- `web/schedule.html` is the single filterable schedule destination. Do not add PGA-only or DP World-only schedule links back into the shared navigation; the legacy tour-specific schedule files remain only for existing bookmarks.
- In `web/schedule.html`, event course names link to Course Details, completed winners link to Player Drilldown, and locations link to Google Maps. Schedule player links must retain the source tour even when the displayed Tour label is `Major`; Player Drilldown should still render that player's betting and archive sections if they are not in the active event field.
...
- Never expose or commit `.env` values. HTTP failures from DataGolf calls must go through `scripts/request_safety.py`, which redacts credential-bearing URLs before errors reach local or Actions logs.
...
- Model refresh schedule: every 2 hours Monday-Thursday at minute 23 UTC. Thursday coverage prevents a new opening-day event from retaining previous-week published assets.
...
- `scripts/run_weekly_all.py` must fetch historical rounds even in `--fast` mode: those records supply the previous-winner scores displayed on PGA and DP World pages. Only the slower course-fit and course-history feature steps are skipped in fast mode. DataGolf provides `course_par` on individual round records, so historical winner scores must total each winner's actual round pars rather than assume an event-level par.
- Course-fit weights are browser-consumed JSON. `scripts/build_course_fit_from_history.py` must serialize non-finite numeric values (for example unavailable driving means/standard deviations) as JSON `null`, never `NaN`/`Infinity`; those literals cause `JSON.parse` to reject the complete asset and make the model summary report that weights are unavailable.
...
- Local edits should usually be committed with `scripts/commit_and_push.sh "message"`, which stages edits, leaves ignored `.env` and Office lock files unstaged, rebases over workflow commits on `origin/master`, commits, rebases once more, and pushes.
...
- `scripts/build_prediction_accuracy.py` writes player-level joins plus event-level Brier/log-loss output (`prediction_accuracy_by_event.csv`), calibration buckets, and equal-field baselines. Brier skill below zero means the current model loses to that no-skill baseline; treat that as a calibration/reweighting signal, not evidence of betting edge. Market comparisons require timestamped, vig-adjusted odds and must not be inferred from the manual betting spreadsheet.
- Initial snapshots now contain provenance hashes for core model code, configuration, and copied artifacts. Use them to identify exactly what produced a frozen prediction before comparing it with results or prices.
