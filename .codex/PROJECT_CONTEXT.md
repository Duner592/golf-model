...
- `web/player.html` is a static player drilldown page. PGA/DP World leaderboard player names link to it with `tour`, `event_id`, `player`, and optional `snapshot=initial`; the page reads served model JSON, archive JSON, and `web/spreadsheet_data.csv`.
