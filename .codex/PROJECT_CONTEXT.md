...
- `web/index.html` shows Featured Current Bets only for pending current-event Outright and E/W bets, because `web/odds_value.html` evaluates only those markets. The green card previews up to three bets per event (player, market, price, bookmaker) and identifies current events from the served tour schedule before deep-linking to Odds & Value with pending, tour, and tournament filters set from URL parameters. First-round-leader bets remain available through the normal pending-bets and betting-history views and must not be presented as Odds & Value bets.
