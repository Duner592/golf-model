...
- Course-history summaries should include `player_name` where historical source data provides it. `scripts/build_course_history_from_hist.py` carries names into `event_{event_id}_course_history_stats.parquet`, and `scripts/build_web_assets.py` can backfill names from the historical combined parquet when older stats files contain only player IDs. `web/event_drilldown.html` displays names in the course-history side card and only falls back to current field/leaderboard lookup when summaries lack names.
