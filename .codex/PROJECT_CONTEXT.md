...
- `scripts/update_archived_event.py` skips previous-week events that do not already have a committed `web/archive/{year}/{slug}/tournament_summary.json`. This prevents archive-update workflow failures when a live event artifact exists under `web/{tour}/events/` but no prediction archive snapshot was committed.
