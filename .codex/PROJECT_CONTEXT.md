...
- `web/odds_value.html` force-refreshes `web/spreadsheet_data.csv` on load and resets its browser controls to Pending/default filters so newly added pending bets are not hidden by stale `localStorage` or restored form state. Shared `web/assets/js/betting-data.js` should fetch fresh CSV data when response metadata is unavailable, falling back to stored data only if a fresh fetch fails.
