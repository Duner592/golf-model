...
- `web/index.html` reads each tour's `meta.json`; when `active_events` contains more than one event, the homepage shows an event switcher and loads the selected preview from that event's `resources` paths under `web/{tour}/events/event_{event_id}/`. Each selected preview exposes an `Event Drilldown` button that links to `web/event_drilldown.html` with the selected `tour`, `event_id`, and course. The homepage checks the served tour schedule for a current-week event and removes both `Model Results` and `Event Drilldown` actions for tours with no current event. Off-week tour sections switch to a compact status card with a schedule link instead of rendering empty model, field, and weather panels.
